...
import { Router, Copy, Check, Key, Terminal, ExternalLink, Loader2 } from "lucide-react";
...
export default function RouterLink() {
  ...
  const generateKey = async () => {
    ...
    try {
      ...
      await axios.put(`${API}/tenants/${tenantId}`, { settings: { api_key: key } }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setApiKey(key);
        ...
        <CardContent className="space-y-4">
          {apiKey ? (
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" onClick={copyApiKey} className="gap-2 border-zinc-700/50 text-zinc-300 shrink-0">
                <Copy className="w-4 h-4" /> Copy
              </Button>
            </div>
          ) : (
            <p className="text-zinc-500 text-sm">No API key yet. Generate one to get started.</p>
          )}
...
/file remove "install.rsc"`}
      ...
      <Card className="bg-zinc-900/60 border-zinc-800/50">
        <CardHeader>
          <CardTitle className="text-white">What this configures on your router</CardTitle>
        </CardHeader>
        <CardContent>

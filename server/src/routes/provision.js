...
router.get("/enroll/bootstrap/:token", async (req, res) => {
  try {
    ...
    const script = [
      ...
      "    :local gwIp [/ip route get ([:pick $defRoutes 0]) gateway]",
      '    :put "[ZTP] Default route gateway: $gwIp"',
      "    :if ([:len [/interface find name=$gwIp]] > 0) do={",
      "      :set wanPort $gwIp",
      "      :set foundWan true",
...
router.get("/v1/:slug/install", async (req, res) => {
  try {
    ...
    const script = [
      ...
      ':put "[Setup] Checking management credentials..."',
      ":global ztpMgmtUser; :global ztpMgmtPass;",
      ':local hasCreds [:len $ztpMgmtUser]',
      ':if ($hasCreds > 0) do={ :put "[Setup] Management credentials: found" } else={ :put "[Setup] No credentials set - add later via Routers page" }',
      "",
      "# ── System Identity ──",
      ':put "[Setup] Setting system identity..."',
      `:if ([/system identity get name] != "${routerIdentity}") do={ /system identity set name="${routerIdentity}"; :put "[Setup] Identity set to: ${routerIdentity}" } else={ :put "[Setup] Identity already correct" }`,
      "",
      "# ── Timezone ──",
      ':put "[Setup] Setting timezone..."',
      ':do { /system clock set time-zone-name=Africa/Nairobi } on-error={ :put "[Setup] Timezone skipped" }',
      "",
      "# ── DNS Configuration ──",
      ':put "[Setup] Configuring DNS..."',
      ':local dnsServers [/ip dns get servers]',
      ':if ($dnsServers != "8.8.8.8,8.8.4.4") do={ /ip dns set servers=8.8.8.8,8.8.4.4; :put "[Setup] DNS set to Google" } else={ :put "[Setup] DNS already configured" }',
      ':do { /ip dns set allow-remote-requests=yes cache-size=10000KiB } on-error={}',
      "",
      "# ── NTP Configuration ──",
      ':put "[Setup] Configuring NTP..."',
      ':do { /system ntp client set enabled=yes mode=unicast servers=pool.ntp.org } on-error={ :put "[Setup] NTP skipped" }',
      "",
      "# ── RADIUS ──",
      ':put "[Setup] Configuring RADIUS..."',
      `:do { /radius add address=${radiusServer} secret="${radiusSecret}" service=ppp,hotspot timeout=300ms comment="ISP RADIUS" disabled=no } on-error={ :put "[Setup] RADIUS exists or skipped" }`,
      "",
      "# ── PPPoE Server ──",
      ':put "[Setup] Configuring PPPoE server..."',
      ":do { :if ([:len [/interface pppoe-server server find]] = 0) do={ /interface pppoe-server server add service-name=pppoe-internet interface=bridge1 authentication=pap,chap,mschap1,mschap2 one-session-per-host=yes disabled=no; :put '[Setup] PPPoE server created' } else={ :put '[Setup] PPPoE server already exists' } } on-error={ :put '[Setup] PPPoE skipped' }",
      "",
      "# ── Hotspot Server ──",
      ':put "[Setup] Configuring Hotspot server..."',
      ":do { :if ([:len [/ip hotspot find]] = 0) do={ /ip hotspot add interface=bridge1 address-pool=dhcp_pool disabled=no; :put '[Setup] Hotspot server created' } else={ :put '[Setup] Hotspot server already exists' } } on-error={ :put '[Setup] Hotspot skipped' }",
      "",
      "# ── Firewall Rules ──",
      ':put "[Setup] Configuring firewall..."',
      ":do {",
      `  /ip firewall filter remove [find comment="ISP Input Established"]`,
      `  /ip firewall filter remove [find comment="ISP Input Invalid"]`,
      ...
      "",
      '  /ip firewall filter add chain=input action=accept connection-state=established,related,untracked comment="ISP Input Established" place-before=0',
      '  /ip firewall filter add chain=input action=drop connection-state=invalid comment="ISP Input Invalid"',
      ...
      "",
      '  :put "[Setup] Firewall rules configured"',
      "} on-error={ :put '[Setup] Firewall configuration failed' }",
      "",
      "# ── NAT Masquerade ──",
      ':put "[Setup] Configuring NAT masquerade..."',
      ":do { /ip firewall nat remove [find comment=\"ISP Masquerade\"]; /ip firewall nat add chain=srcnat action=masquerade out-interface=ether1 comment=\"ISP Masquerade\"; :put '[Setup] NAT masquerade added' } on-error={ :put '[Setup] NAT skipped' }",
      "",
      "# ── Connection Tracking ──",
      ':put "[Setup] Tuning connection tracking..."',
      ':do { /ip firewall connection tracking set tcp-close-wait-time=10s tcp-time-wait-time=10s } on-error={}',
      "",
      "# ── Secure Services ──",
      ':put "[Setup] Securing services..."',
      ':do { /ip service set telnet disabled=yes; /ip service set ftp disabled=yes; /ip service set www disabled=yes; /ip ssh set strong-crypto=yes; :put "[Setup] Insecure services disabled, SSH hardened" } on-error={}',
      "",
      "# ── Report back to billing server ──",
      ...
      `:do { /tool fetch url=$reportUrl http-header-field="Authorization: Bearer ${apiKey}" mode=${fetchMode} ${certFlag} output=none } on-error={ :log warning "[Setup] Report failed" }`,
      "",
      "# ── Schedule auto-sync ──",
      "/system scheduler remove [find name=billing-sync]",

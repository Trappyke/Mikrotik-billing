...
router.get("/v1/:slug/install", async (req, res) => {
  try {
    ...
    const script = [
      ...
      "",
      "# ── NAT Masquerade ──",
      ':do { /ip firewall nat remove [find comment="ISP Masquerade"]; /ip firewall nat add chain=srcnat action=masquerade out-interface=ether1 comment="ISP Masquerade" } on-error={}',
